...
测试阿里云百炼 wanxiang-image-edit 模型。
"""
import asyncio
import sys
import base64
...
async def test_aliyun_edit():
    ...
    # 构建提示词
    from app.tasks.generation_tasks import _build_aliyun_prompt
    prompt = _build_aliyun_prompt(ocr_blocks, edit_blocks, img_width, img_height, "zh", visual_styles)
    print(f"Prompt:\n{prompt}\n")

    # 调用阿里云百炼
    aliyun = AliyunClient()
    print("Calling Aliyun wanxiang-image-edit...")
    result_b64 = await aliyun.edit_image(
    ...
    with open("/tmp/aliyun_gen.png", "wb") as f:
        f.write(result_bytes)
    print("Image saved to /tmp/aliyun_gen.png")
